...
export class WebSocketService {
  ...
  sendMessage<T extends IPacket>(message: T):void {
    ...
    // 获取请求包packetId
    let id = PacketId.class2PacketId.get((message as IPacket).getClassName());
    // 拿到protobufjs生成的字节数组，封装成ReqPakcet
    let root=new Root();
    load(`src/assets/proto/${(message as IPacket).getClassName()}.proto`, root ,(err, root)=> {
      if (err)
        throw err;
      ...
      let result = proxy.create(message);
      console.log(`message = ${JSON.stringify(result)}`);

      let buffer = proxy.encode(message).finish();
      console.log(`buffer = ${Array.prototype.toString.call(buffer)}`);

      let decoded = proxy.decode(buffer);
      console.log(`decoded = ${JSON.stringify(decoded)}`);
      let reqPacket=ReqPakcet.valueOf(id,buffer);
